...
export default function EditJadwal() {
  ...
  useEffect(() => {
    const fetchCurrentJadwal = async () => {
      try {
        ...
        const res = await API.get(`/jadwal-index/${id}`);
        const data = res.data;
  ...
  const onSubmit = async (data: JadwalForm) => {
    try {
      ...
      console.error(error);
      const errorMessage =
        error.response?.data?.message || "Gagal memperbarui data jadwal";
      alert(errorMessage);
    } finally {
      setLoading(false);
    }
    ...
    <div className="p-2">
      ...
      <div className="border-b border-gray-100 p-6">
        ...
        <p className="text-gray-500 mt-1">
          Perbarui detail acara, pergeseran waktu, atau pemindahan lokasi agenda
          kegiatan.
        </p>
      </div>
